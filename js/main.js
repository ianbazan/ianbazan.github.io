...
(function ($) {
  ...
  /* project links open lightbox
   * -------------------------------------------------- */
  const ssProjectLinks = function () {
    $(".folio-item__project-link").on("click", function (e) {
      const href = $(this).attr('href') || '';
      // If the link is an external URL, allow the default navigation (e.g. open in new tab)
      if (/^https?:\/\//i.test(href)) {
        return;
      }
      e.preventDefault();
      const $folio = $(this).closest(".folio-item");
      const $thumb = $folio.find(".folio-item__thumb-link").first();
      if ($thumb && $thumb.length) $thumb.trigger("click");
    });
  };
